...
Uso:
  python3 generator/generate-dungeon.py [opzioni] --output mappa.png
...
def generate(gw, gh, n_rooms, rng, room_min=4, room_max=12, corridor_width=2, corridor_rows=1):
        ...
        for start_x in range(gw):
            ...
            for idx in list(room_by_cell.keys()):
                ...
                if r['type'] == 'corridor':
                    ...
                    # If at least one cell of this room is in the component
    ...
    # Connect them with L-shaped corridors to the nearest reached room
    ...
    for a, b in edges:
        ...
        if ra[0] == rb[0]:
            ...
            if abs(dc) == 1:
                ...
                # Corridor — use already computed mid_y
                corr_mid_y = mid_y
                cid = f'C{len(corridors)+1}'
...
def main():
    ...
    # External passage: on the room with most exterior exposure
    ...
    entrance_door_pos = None
    if best_entrance and exterior_exposure(best_entrance) > 0:
        entrance_door_pos = add_entrance_door(grid, (best_entrance['x'], best_entrance['y'], best_entrance['w'], best_entrance['h']))
    ...
    import datetime
    # Raccogli posizioni passage dalla griglia: celle CORR adiacenti a FLOOR o EXTERIOR
    gh_grid, gw_grid = len(grid), len(grid[0])
    passages_list = []
    seen_passages = set()
    for y in range(1, gh_grid-1):
        for x in range(1, gw_grid-1):
            if grid[y][x] != CORR:
                continue
            l, r = grid[y][x-1], grid[y][x+1]
            u, d_cell = grid[y-1][x], grid[y+1][x]
            has_floor_h = (l in (FLOOR, EXTERIOR) or r in (FLOOR, EXTERIOR))
            has_floor_v = (u in (FLOOR, EXTERIOR) or d_cell in (FLOOR, EXTERIOR))
            if has_floor_h or has_floor_v:
                orient = 'h' if has_floor_v and not has_floor_h else 'v'
                key = (x, y)
                if key not in seen_passages:
                    seen_passages.add(key)
                    passages_list.append({'x': x, 'y': y, 'orient': orient})
    # Passage esterno
    if entrance_door_pos:
        ex, ey = entrance_door_pos
        u = grid[ey-1][ex] if ey > 0 else EXTERIOR
        d_cell = grid[ey+1][ex] if ey < gh_grid-1 else EXTERIOR
        orient = 'h' if u == FLOOR or d_cell == FLOOR else 'v'
        if (ex, ey) not in seen_passages:
            passages_list.append({'x': ex, 'y': ey, 'orient': orient, 'external': True})

    # Calcola width per ogni passage (numero di celle consecutive stesso orient)
    p_map = {(p['x'], p['y']): p for p in passages_list}
    seen_w = set()
    for p in passages_list:
        ...
        if key in seen_w:
            continue
        orient = p['orient']
        # Trova tutte le celle consecutive
        cells = [key]
        seen_w.add(key)
        nx, ny = (p['x']+1, p['y']) if orient == 'h' else (p['x'], p['y']+1)
        while (nx, ny) in p_map and p_map[(nx, ny)]['orient'] == orient:
            cells.append((nx, ny))
            seen_w.add((nx, ny))
            if orient == 'h': nx += 1
            else: ny += 1
