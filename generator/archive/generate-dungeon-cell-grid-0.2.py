...
def generate(gw, gh, n_rooms, rng, room_min=4, room_max=12, corridor_width=2):
    grid = [[EXTERIOR] * gw for _ in range(gh)]

    # ── Passo 1: costruisci griglia di celle con dimensioni variabili ─────────
    # Usa room_min/room_max direttamente per massima varietà di dimensioni
    cells = []
    cell_grid = []

    y = 1
    while y + room_min + 1 < gh:
        ch = rng.randint(room_min, min(room_max, gh - y - 1))
        row_cells = []
        x = 1
        while x + room_min + 1 < gw:
            cw = rng.randint(room_min, min(room_max, gw - x - 1))
        ...
        for ry in range(y, y+ch):
            for rx in range(x, x+cw):
                if 0 <= ry < gh and 0 <= rx < gw:
                    grid[ry][rx] = FLOOR
        r = {'id': f'S{i+1}', 'x': x, 'y': y, 'w': cw, 'h': ch, 'type': 'room', 'connections': []}
        rooms.append(r)
        room_by_cell[idx] = r
        ...
        if ra[0] == rb[0]:
            ...
            if abs(dc) == 1:
                if oy2 <= oy1:
                    ...
                    # Segmento verticale dal centro di a al centro di b
...
def main():
    ...
    # Per entrance: scegli la stanza con più quadretti di muro adiacenti a EXTERIOR
    ...
    for sp in sp_queue:
        if sp == 'entrance':
            best = max((i for i in idxs if i not in used), key=lambda i: exterior_exposure(rooms[i]), default=None)
            if best is not None:
                used.add(best)
                rooms[best]['special'] = 'entrance'
        else:
            for i in idxs:
                if i not in used:
                    used.add(i)
    ...
    # specials per il rendering (compatibilità)
    specials = [(r['x'], r['y'], r['w'], r['h'], r['special']) for r in rooms if 'special' in r]

    # Porta verso l'esterno sulla stanza entrance
    entrance_room = next((r for r in rooms if r.get('special') == 'entrance'), None)
    if entrance_room:
        add_entrance_door(grid, (entrance_room['x'], entrance_room['y'], entrance_room['w'], entrance_room['h']))
        entrance_room['connections'].append({'to': 'esterno', 'via': 'porta esterna'})

    # Rileva porte esterne su tutte le stanze (CORR adiacente a EXTERIOR)
    for room in rooms:
        if room.get('special') == 'entrance':
            continue  # già aggiunta
        rx, ry, rw, rh = room['x'], room['y'], room['w'], room['h']
        has_ext = False
        for x in range(rx, rx+rw):
